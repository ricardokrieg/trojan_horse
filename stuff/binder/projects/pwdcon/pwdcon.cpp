...
// PWDCON: Template 
//
// Important: Compiler mas set /O2 (Maximize Speed) to ensure inline functions
//
// Author: Ciro Sisman Pereira
//
...
// *** Offsets indicating data position into vg_data_ep_data
...
// *** Simulates NewEntryPoint
int __stdcall NewEntryPoint();
// *** Simulates OEP (Original Entry Point)
...
void fill_data_areas( char *_pwd )
{
   ...
   for ( int iC = 0; vg_imports[iC].dll; iC++ )
   {
      ...
      iOffSet2 += _GAP_FUNCTION_NAMES; // *** A small gap saparating functions groups
   }
}
